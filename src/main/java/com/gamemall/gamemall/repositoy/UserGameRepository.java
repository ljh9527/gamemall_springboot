...
public interface UserGameRepository  extends JpaRepository<UserGame, Integer>, JpaSpecificationExecutor<UserGame> {
    ...
    @Query(nativeQuery = true, value = " SELECT user_game.playtime,user_game.address,user_game.lastplay,game.id,game.download,game.game_name,game_image.image_cover FROM game_image RIGHT JOIN game ON  game.id = game_image.game_id" +
